...
thermo_data = pandas.read_csv("data/thermo_test.txt", sep=',', header = None).iloc[100::201]
...
thermo_data['p2'] = thermo_data.iloc[:,0].mul(thermo_data.iloc[:,1],axis = 0)
thermo_data['p3'] = thermo_data.loc[:,'p2'].mul(thermo_data.iloc[:,1],axis = 0)
thermo_data['p4'] = thermo_data.loc[:,'p3'].mul(thermo_data.iloc[:,1],axis = 0)
thermo_data['p5'] = thermo_data.loc[:,'p4'].mul(thermo_data.iloc[:,1],axis = 0)

linear =linear_model.LinearRegression(fit_intercept=True)
# linear = SVR(kernel='linear', C=1000,max_iter=100000)
data_X = pandas.concat([thermo_data.iloc[:,0],thermo_data.iloc[:,9:12]],axis=1,ignore_index=True)
...
cons = ({'type': 'eq', 'fun': lambda x: fit(X[0,:], x)-1},
        {'type': 'eq', 'fun': lambda x: fit(X[-1, :], x)})
