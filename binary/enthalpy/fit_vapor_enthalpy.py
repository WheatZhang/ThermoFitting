...
thermo_vapor = pandas.read_csv("data/thin_enthalpy_vapor.csv", sep=',')
x = np.array(thermo_vapor.loc[:,'pressure'])
y = np.array(thermo_vapor.loc[:,'temperature'])
z1 = np.array(thermo_vapor.loc[:,'nitrogen'])
z2 = np.array(thermo_vapor.loc[:,'oxygen'])

create_cubic_polyfit(x,y,z1,"vapor_nitrogen")
create_cubic_polyfit(x,y,z2,"vapor_oxygen")
import vapor_nitrogen
z_hat = np.zeros(shape = z1.shape)
for i in range(z1.size):
    z_hat[i] = vapor_nitrogen.vapor_nitrogen(x[i],y[i])
print(np.max(np.abs(z1-z_hat)))
errFile1 = open('error/csvFile3.csv','w', newline='')
...
for i in range(z2.size):
    z_hat[i] = vapor_oxygen.vapor_oxygen(x[i],y[i])
print(np.max(np.abs(z2-z_hat)))
errFile2 = open('error/csvFile4.csv','w', newline='')
